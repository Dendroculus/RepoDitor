  ...
  return (
    <SkeletonRegion label={label} testId="upgrades-skeleton">
      ...
      <div className="mt-6 grid gap-x-8 gap-y-5 xl:grid-cols-2">
        {[0, 1, 2, 3].map((row) => (
          <div
            className="relative min-h-24 border-t border-line pt-4 pr-24"
            data-skeleton-kind="upgrade-row"
            key={row}
          >
  ...
  if (error) {
    return (
      <section aria-labelledby="upgrades-error-title">
        <h2 className="text-xl font-semibold text-ink" id="upgrades-error-title">
      ...
      {upgrades.length === 0 ? (
        ...
        <div className="mt-6 grid min-w-0 gap-x-8 gap-y-5 xl:grid-cols-2">
          {upgrades.map((upgrade) => {
            ...
            return (
              <div
                className="relative min-h-24 min-w-0 border-t border-line pt-4 pr-24"
                key={upgrade.key}
              >
                <div className="absolute top-4 right-0">
                  <GameIcon
                    ...
                    testId={`upgrade-icon-${upgrade.key}`}
                    token={upgrade.iconToken}
                    variant="upgrade"
                  />
                </div>
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <label
                    className="min-w-0 text-sm font-semibold text-ink"
                    htmlFor={upgrade.key}
                    title={upgrade.label}
                  >
                    {upgrade.label}
                  </label>
                  {upgrade.presentationSource !== "installed" ? (
                ...
                {edit ? (
                  <p
                    ...
                    data-testid={`pending-upgrade-${upgrade.key}`}
                    id={pendingId}
                  >
